...
### 💬 智能对话系统
- **多模型支持**: 兼容 OpenAI、DeepSeek、Anthropic Claude、阿里云百炼、智谱GLM、Google Gemini、硅基流动(SiliconFlow)、火山引擎、百度千帆、Moonshot、MiniMax、Groq、NVIDIA 等30+种LLM提供商
- **实时流式响应**: SSE流式输出，支持文本、推理内容、工具调用的增量展示
- **消息再生**: 支持三种再生模式（覆盖、多版本、追加），灵活管理对话历史
- **会话锁定**: 防止并发请求冲突，确保会话一致性
- **中断处理**: 客户端断开时自动中止LLM请求，节省资源

### 🧠 智能上下文管理
- **自动摘要压缩**: 当对话超出上下文窗口时，自动生成历史摘要并压缩旧消息
...
- **主/子账户**: 支持团队协作、权限分级和数据隔离

### 🤖 多平台机器人集成（Bot Gateway）
GuaDa 支持将 AI 对话能力扩展到多个即时通讯平台：
...
**核心特性**:
- **统一适配架构**: 采用策略模式设计 `IBotPlatform` 接口
- **自动重连机制**: WebSocket 连接断开时自动重试，支持指数退避策略
- **消息合并与缓冲**: 智能合并短时间内的多条消息（1.5秒窗口）
- **多模态消息支持**: 自动下载并存储图片和文件附件，生成预览图
- **流式回复能力**: 支持边生成边推送，实时将 AI 回复发送回聊天平台
- **会话映射管理**: 自动将外部平台用户映射到内部 Session
- **知识库集成**: 支持为不同机器人配置专属知识库
- **实例生命周期管理**: 支持动态启动、停止、重启机器人实例
...
```
 ...
 │                                                             │
 │  AgentService (核心)                                        │
 │  ├── SessionLockService (会话锁控制)                        │
 │  ├── executeAgentLoop (多轮 ReAct 循环)                     │
 │  ├── 迭代次数限制 (MAX_ITERATIONS = 40)                     │
 │  └── 流式事件产出 (create/text/think/tool_call/finish)      │
 │                                                             │
 └────┬──────────────────────────┬─────────────────────────────┘
      │                          │
      ▼                          ▼
 ┌─────────────────────┐ ┌──────────────────────────────────┐
 │ Layer 3a: 上下文管理 │ │ Layer 3b: 工具调用层              │
 ├─────────────────────┤ ├──────────────────────────────────┤
 │                     │ │                                  │
 │ ConversationContext │ │ ToolOrchestrator                 │
 │ Factory             │ │ ├── 工具发现                      │
 │ ├── 上下文初始化     │ │ ├── 工具执行                      │
 │ ├── 消息加载         │ │ ├── 结果格式化                    │
 │ └── 压缩状态恢复     │ │ └── 并行执行支持                  │
 │                     │ │                                  │
 │ MessageStoreService │ │ MCP SDK                          │
 │ ├── 历史消息加载     │ │ ├── Server 连接管理               │
 │ ├── Token 计数       │ │ ├── Tools 定义                    │
 │ └── 压缩触发检测     │ │ └── Prompts 注入                  │
 │                     │ │                                  │
 │ CompressionEngine   │ │ Built-in Tools                   │
 │ ├── 摘要生成         │ │ ├── knowledge_base (知识库检索)   │
 │ ├── 滑动窗口压缩     │ │ ├── shell (命令执行)              │
 │ └── 检查点保存       │ │ ├── time (时间查询)               │
 ...
 │                                                             │
 │  LLM Service                                                │
 │  ├── 模型适配器选择 (OpenAI/Gemini/Claude/DeepSeek等)        │
 │  ├── 参数构建 (messages/tools/temperature/maxTokens)        │
 │  ├── 流式响应解析                                           │
 │  └── Token 统计 (tiktoken/HuggingFace)                      │
 │                                                             │
 └─────────────────────────────────────────────────────────────┘
 ...
 1. 会话锁定机制 (SessionLockService)
    • 确保同一会话同时只有一个请求在处理
    • 防止并发冲突和状态不一致
    • 基于内存 Map 实现，自动释放锁

 2. 多轮 ReAct 循环 (executeAgentLoop)
    • 最大迭代次数：40 次（防止无限循环）
    • 每轮流程：LLM 推理 → 工具调用 → 结果注入 → 下一轮推理
    • 支持思维链（Thinking Chain）输出
    • 工具调用结果作为新消息加入上下文

 3. 上下文管理 (ConversationContext + MessageStore)
    • 动态加载历史消息（根据 Token 限制）
    • 自动触发摘要压缩（达到阈值时）
    • 压缩状态持久化（SessionContextState 表）
    • 支持检查点恢复（断点续传）

 4. 工具调用流程 (ToolOrchestrator)
    • 工具定义注入：将可用工具列表传递给 LLM
    • 工具提示词附加：指导模型正确使用工具
    • 增量累积：流式接收工具调用参数
    • 并行执行：多个工具调用可并行执行
    • 结果清理：工具输出经过格式化和截断
