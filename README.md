...
> 智能 AI 对话系统，支持 ReAct Agent、多模型适配、RAG 知识库检索、MCP 工具调用与 Skills 技能框架。目标是打造一个可用、易用、好用的个人智能助理与企业 AI 中台。**目前处于早期开发阶段，部分功能正在持续快速迭代中。**
...
GuaDa 是一个功能完备的 AI 对话和工作辅助平台，集成了**浏览器自动化**、 **RAG 知识库**、**MCP 协议** 和 **Skills 技能框架**，支持多种模型提供商，可作为企业内部 AI 中台、聊天机器人后端或个人智能助理使用。
...
| ![产品截图5](./images/5.png) | ![工作目录](./images/工作目录.png) |
...
可编排、可扩展的 Agent 框架，基于 **ReAct (Reasoning + Acting)** 模式实现多轮自治循环。Agent 可自主调度知识检索、工具执行、技能调用等能力，支持会话锁、流式传输、中断处理与再生模式。Agent 编排和子 Agent 能力正在开发中。

### 知识库

- **语义 + 关键词双重匹配搜索**：融合向量余弦相似度（sqlite-vec）与 FTS5 全文检索 + jieba 中文分词 + BM25 重排序，加权计算最终得分，精度更高
- **目录层级管理**：支持层级目录结构，可创建文件夹、批量上传、移动、重命名
- **Agent 自主多轮搜索**：Agent 可自主决定搜索策略、轮次、文件和分块，而非系统预先排序
- **Agent 自主添加文档**：Agent 读取本地文件后自动解析、分块、向量化并存入知识库，全程无需用户手动操作

### 机器人

支持 QQ、企业微信等 IM 平台接入，通过统一适配接口（`IBotPlatform`）实现多平台无缝接入。配合知识库可轻松搭建智能客服系统，支持自动重连、消息合并、会话映射与生命周期管理。

### 长期记忆

- **对话记忆持久化**：基于 `MemoryToolProvider` 实现会话记忆读写，关键记忆长期备份
- **长上下文自动压缩**：采用两级压缩策略，优先裁剪工具结果再进行语义压缩，支持检查点持久化与压缩回退
- **保护机制**：最近 5 条消息强制保留，最新 3 条工具结果免于裁剪

### 技能管理

可配置的技能（Skill）体系，基于 **Anthropic Skills 协议** 设计，以文件系统为存储，支持热插拔和渐进式加载。技能通过 `SkillToolBridgeService` 注册为 `skill__` 命名空间工具，扩展 Agent 能力边界。

### 多部署方式

支持 **Electron 桌面端**、**Web 应用**、**Docker 容器化** 三种部署方式，灵活适配个人使用、企业内网部署与云端部署场景。

### 企业级数据支持

默认使用 **SQLite** 数据库，通过 Prisma ORM 可无缝切换至 **MySQL**、**PostgreSQL** 等高级数据库，满足企业级数据存储需求。

### 定时任务

Agent 可根据定时任务周期执行，支持配置执行时间、周期与角色模型。晚上值班监督任务进展、定时巡检、定时报告生成等场景毫无压力。

### 浏览器自动化

Electron 模式下内嵌 Chromium 浏览器引擎，Agent 可直接操控浏览器页面，实现网页浏览、数据采集、页面交互等高级任务，打破传统 API 的边界限制。

### 多模型管理

统一接入和管理多种大语言模型（LLM），兼容 OpenAI、Anthropic、Azure、Google 等主流供应商，支持灵活切换与模型级配置。
