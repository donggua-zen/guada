...
无论你是写脚本追热点的**新媒体人**、想让 Agent 代写代码的**程序员**、需要它记住人物设定的**创作者**，还是想搭一个绑定知识库、接入 IM 的内部 AI 助手的**团队**——GuaDa 都能直接上手。
...
| **定时任务** | ✅ Cron·会话隔离·执行记录 | ❌ | ❌ | ❌ |
| **🤖 多 Agent** | ✅ 全能力独立配置（角色/模型/工具/知识库） | ⚠️ 仅子代理·限 Claude 模型·不可嵌套 | ⚠️ 仅子代理·不可嵌套 | ❌ |
| **多模型支持** | ✅ 17 家厂商·含国产模型 | ❌ 仅 Claude | ❌ 仅 OpenAI | ⚠️ |
| **数据本地可控** | ✅ | ⚠️ | ⚠️ | ⚠️ |
| **配置难易**（⭐越多越简单） | ⭐⭐⭐⭐ 开箱即用 | ⭐⭐⭐ 登录即用 | ⭐⭐⭐ API Key 即用 | ⭐⭐⭐⭐⭐ 登录即用 |
...
| **🌐 浏览器自动化** | Electron 内嵌 Chromium，Agent 实时可视操控。**6 层智能压缩**，省 70%+ Token 不丢结构 | [→ 详情](docs/browser-automation.md) |
