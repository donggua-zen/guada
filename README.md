...
GuaDa 是一个现代化的全栈AI对话应用,采用前后端分离架构。后端基于 NestJS + TypeScript 构建,前端使用 Vue 3 + Vite,同时支持 Electron 桌面应用打包。系统支持多种LLM提供商(OpenAI、阿里云、智谱等),集成了RAG知识库、MCP协议工具调用、会话上下文压缩、多轮工具调用等高级特性。
...
graph TB
    subgraph "客户端层"
        Web[Web 浏览器<br/>Vue 3 + Vite]
        Electron[Electron 桌面应用]
    end

    subgraph "后端服务层 - NestJS"
        API[API Gateway<br/>REST + SSE]
        
        subgraph "核心业务模块"
            Chat[Chat Module<br/>对话引擎]
            KB[Knowledge Base<br/>知识库 RAG]
            BotGW[Bot Gateway<br/>多平台机器人]
            Tools[Tools & MCP<br/>工具调用]
        end
        
        subgraph "支撑模块"
            Auth[Auth Module<br/>JWT 认证]
            Models[LLM Core<br/>多模型适配]
            Files[File Service<br/>文件管理]
            Users[User Management<br/>用户管理]
        end
    end

    subgraph "数据层"
        SQLite[(SQLite<br/>主业务数据库)]
        VectorDB[(Vector DB<br/>sqlite-vec)]
        FileStore[File Storage<br/>静态文件]
    end
    ...
    subgraph "外部服务"
        ...
        WeCom[企业微信]
        Lark[飞书/Lark]
        LLMs[LLM Providers<br/>OpenAI/Gemini/智谱等]
