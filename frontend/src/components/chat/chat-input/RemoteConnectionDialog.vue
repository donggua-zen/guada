  ...
  >
    ...
    <template v-if="!editing">
      <div v-if="connections.length > 0" class="flex flex-col gap-1 mb-3">
        ...
        >
          ...
          <el-button text size="small" @click.stop="editConnection(conn)">{{ t('common.edit') }}</el-button>
          <el-button text size="small" type="danger" @click.stop="removeConnection(conn.id)">{{ t('common.delete') }}</el-button>
        </div>
      </div>
      <div v-else class="text-center py-6 text-(--color-text-gray)">
        <p class="text-sm">{{ t('chat.input.noRemoteConnection') }}</p>
      </div>
      <el-button @click="startNew" type="primary" plain class="w-full">
        {{ t('chat.input.newConnection') }}
      </el-button>
    </template>
    ...
    <template v-else>
      ...
      <div class="flex flex-col gap-0.5 mb-3">
        <label class="text-xs font-medium">{{ t('chat.input.connectionName') }} <span class="text-red-500">*</span></label>
      ...
      <div v-if="!editConnId" class="flex flex-col gap-0.5 mb-3">
        ...
        <el-select v-model="editScheme" :placeholder="t('chat.input.selectConnectionType')" class="w-full">
          <el-option
      ...
      <div v-if="testResult" class="mt-3 text-sm" :class="testResult.success ? 'text-green-500' : 'text-red-500'">
        {{ testResult.success ? t('chat.input.connectionSuccess') : `✗ ${testResult.error}` }}
      </div>

      <!-- 按钮 -->
      <div class="flex justify-between mt-4">
        <el-button @click="testConnection" :loading="testing" :disabled="!editScheme">
          {{ t('chat.input.testConnection') }}
        </el-button>
...
async function saveConnection() {
  if (!editName.value.trim()) {
    ElMessage.warning(t('chat.input.fillConnectionName'))
    return
  }
  saving.value = true
  try {
