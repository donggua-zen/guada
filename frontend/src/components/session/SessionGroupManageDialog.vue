  ...
  <el-dialog v-model="dialogVisible" :title="t('session.group.title')" width="420px" :close-on-click-modal="false"
    ...
    <div class="flex flex-col gap-3">
      ...
      <div class="flex gap-2">
        <el-input v-model="newGroupName" :placeholder="t('session.group.newPlaceholder')" maxlength="20" show-word-limit
          @keyup.enter="handleCreateGroup" />
        <el-button type="primary" @click="handleCreateGroup" :disabled="!newGroupName.trim()">
          {{ t('session.group.create') }}
        </el-button>
      </div>
            ...
            <template #item="{ element, index }">
              <div
                ...
                <template v-if="editingId === element.id">
                  ...
                  <el-button type="primary" size="small" @click="saveEdit">{{ t('session.group.save') }}</el-button>
                  <el-button size="small" @click="cancelEdit">{{ t('session.group.cancel') }}</el-button>
                </template>
      ...
      <div class="text-xs text-gray-400 leading-relaxed">
        <p>{{ t('session.group.tipsTitle') }}</p>
        <ul class="list-disc list-inside mt-1 space-y-0.5">
          <li>{{ t('session.group.tip1') }}</li>
          <li>{{ t('session.group.tip2') }}</li>
          <li>{{ t('session.group.tip3') }}</li>
        </ul>
      </div>
    </div>
  </el-dialog>
...
const handleCreateGroup = async () => {
  ...
  if (group) {
    toast.success(t('session.group.createSuccess'))
    newGroupName.value = ''
  } else {
    toast.error(t('session.group.createFailed'))
  }
}
...
const saveEdit = async () => {
  ...
  if (!name) {
    toast.error(t('session.group.nameEmpty'))
    return
  }

  const success = await sessionGroupStore.updateGroup(editingId.value, name)
  if (success) {
    toast.success(t('session.group.updateSuccess'))
    editingId.value = null
  } else {
    toast.error(t('session.group.updateFailed'))
  }
}
...
const handleDeleteGroup = async (group: SessionGroup) => {
  const confirmed = await confirm(t('session.group.deleteTitle'), t('session.group.deleteConfirm', { name: group.name }), {
    type: 'warning',
  ...
  if (confirmed) {
    ...
    if (success) {
      toast.success(t('session.group.deleteSuccess'))
    } else {
      toast.error(t('session.group.deleteFailed'))
    }
  }
}
...
const onDragEnd = async () => {
  ...
  if (success) {
    toast.success(t('session.group.reorderSuccess'))
  } else {
    toast.error(t('session.group.reorderFailed'))
    // 恢复原始顺序
    localGroups.value = [...groups.value]
  }
}
