...
class RAGHelper:
    ...
    def add_memory_from_messages(self, session_id, user_message, assistant_message):
        """从消息中添加记忆"""
        # 删除上一次用户消息对应的记忆
        self.vector_memory.delete_memory_by_message_id(message_id=user_message["id"])
    ...
    def query_relevant_memories(
        ...
        memory_results = self.vector_memory.query_memories(
...
class MemoryStrategy(ABC):
    ...
    @abstractmethod
    def process_memory(
        self, session_id: str, character: dict, current_message_id: str
    ) -> Tuple[List[dict]]:
        """
        处理记忆逻辑
...
class SlidingWindowStrategy(MemoryStrategy):
    ...
    def process_memory(self, session_id: str, character: dict, current_message_id: str):
...
class SummaryAugmentedSlidingWindowStrategy(MemoryStrategy):
    ...
    def process_memory(
        ...
        start_id = summary_service.get_last_message_id(session_id)
        messages = message_service.get_messages(
            session_id=session_id,
        ...
        compression_result = summary_service.conditionally_compress_history(
            session_id, character=character, messages=messages
        )
