...
export class UploadPathService {
    ...
    constructor(private configService: ConfigService) {
        ...
        this.baseDir = this.configService.get<string>('upload.baseDir') || './static/file_stores';
        // Web 访问前缀 (例如: /static/file_stores)
        this.publicPrefix = this.configService.get<string>('upload.publicPrefix') || '/static/file_stores';
    }
    ...
    getPathFromWebUrl(webUrl: string): string | null {
        ...
        // 提取相对路径部分
        const relativePath = webUrl.substring(this.publicPrefix.length).replace(/^\//, '');
