...
export class McpClientService {
    ...
    private async withClient<T>(
        ...
        try {
            ...
            this.logger.error(`MCP client operation failed: ${error.message}`, error.stack);
            throw error;
        } finally {
            // 确保连接被关闭
            if (client) {
                try {
                    ...
                    this.logger.warn(`Failed to close MCP connection: ${closeError.message}`);
                }
            }
        }
    }
    ...
    async listTools(config: McpServerConfig): Promise<Record<string, any>> {
        return this.withClient(config, async (client) => {
            ...
            this.logger.log(`Fetched ${Object.keys(toolsDict).length} tools from ${config.url}`);
            return toolsDict;
        });
    }
    ...
    async callTool(
        ...
        try {
            return await this.withClient(config, async (client) => {
                ...
                if (result && typeof result === 'object') {
                    ...
                    if ('content' in result && Array.isArray(result.content)) {
                        content = result.content.map((item: any) => item.text || JSON.stringify(item)).join('\n');
                    } else {
            ...
            this.logger.error(`Failed to call MCP tool '${toolName}': ${error.message}`);
            return {
                success: false,
                error: error.message,
    ...
    async healthCheck(config: McpServerConfig): Promise<boolean> {
        try {
            ...
            this.logger.warn(`Health check failed for ${config.url}: ${error.message}`);
            return false;
        }
    }
}
