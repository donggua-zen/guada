...
export class FileParserService {
    ...
    async detectFileType(fileExtension: string): Promise<string> {
        ...
        if (this.TEXT_EXTENSIONS.has(ext)) {
            ...
            return 'word';
        } else if (['py', 'js', 'ts', 'java', 'cpp', 'c', 'go', 'rs'].includes(ext)) {
            return 'code';
        } else {
            return 'unknown';
        }
    ...
    async parseFile(
        ...
        try {
            ...
            this.logger.error(`文件解析失败：${fileExtension}, size=${fileSize}, error=${error.message}`);
    ...
    private validateFileSize(fileSize: number, fileType: string): void {
        ...
        if (fileSize > limit) {
            throw new Error(`文件超出大小限制（最大 ${Math.floor(limit / 1024 / 1024)}MB）`);
        }
    }
