...
export class ToolOrchestrator {
    ...
    async getAllToolPrompts(context: any): Promise<string> {
        ...
        for (const [namespace, provider] of this.providers.entries()) {
            try {
                ...
                this.logger.error(`Error getting prompt from provider ${namespace}: ${error.message}`);
            }
        }
    ...
    async executeBatch(requests: ToolCallRequest[], context: any): Promise<ToolCallResponse[]> {
    ...
    private async execute(request: ToolCallRequest, context: any): Promise<ToolCallResponse> {
        const namespace = request.name.split('__')[0];
        const provider = this.providers.get(namespace);

        if (!provider) {
            throw new Error(`${request.name} No provider found for namespace: ${namespace}`);
        }
