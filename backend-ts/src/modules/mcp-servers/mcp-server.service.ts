  ...
  private async fetchToolsFromServer(
    ...
    try {
      ...
      if (Object.keys(toolsDict).length > 0) {
        this.logger.log(
          `Successfully fetched ${Object.keys(toolsDict).length} tools from ${serverUrl}`,
        );
      } else {
        this.logger.warn(
          `No tools found at ${serverUrl}. You can manually configure tools later.`,
        );
      }
      ...
      this.logger.warn(
        `Failed to automatically fetch tools from ${serverUrl}: ${error.message}`,
      );
      this.logger.log(
