...
import { GoogleGenerativeAI, HarmCategory, HarmBlockThreshold, SchemaType, GenerateContentResult } from '@google/generative-ai';
    ...
    async *chatCompletion(params: LLMCompletionParams): AsyncGenerator<LLMResponseChunk> {
        ...
        try {
            ...
            const contentToSend = Array.isArray(lastMessage?.content)
                ? lastMessage.content.map(p => p.type === 'text' ? { text: p.text } : p as any)
                : lastMessage?.content || '';
            ...
            for await (const chunk of result.stream) {
                ...
                if (functionCalls && functionCalls.length > 0) {
                    responseChunk.toolCalls = functionCalls.map((fc, index): ToolCallItem => ({
