...
import { MessageRecord, InternalToolDefinition, LLMCompletionParams, LLMResponseChunk, ToolCallItem } from '../types/llm.types';
...
export class OpenAIAdapter implements LLMAdapter {
    ...
    private createClient(providerConfig: any) {
        return new OpenAI({
            ...
            apiKey: providerConfig?.apiKey || process.env.OPENAI_API_KEY || 'sk-placeholder',
        });
    }
    ...
    private async *handleStreamResponse(response: any): AsyncGenerator<LLMResponseChunk> {
        for await (const chunk of response) {
            ...
            if (delta?.tool_calls) {
                responseChunk.toolCalls = delta.tool_calls.map((tc): ToolCallItem => ({
                    id: tc.id,
                    index: tc.index,
                    type: 'function',
    ...
    private handleNonStreamResponse(response: any) {
        ...
        if (!choice || !choice.message) throw new Error('Invalid response from LLM API');
    ...
    private handleError(error: any, isStream: boolean) {
        ...
        if (error.message.includes('timeout')) throw new Error('LLM request timed out (60s)');
        throw error;
    }

    private cleanup(response: any) {
        if (response && typeof response.controller?.abort === 'function') {
            try { response.controller.abort(); } catch (e) { /* ignore */ }
        }
    }
}
