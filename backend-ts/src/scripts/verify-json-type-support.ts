...
async function main() {
  ...
  try {
    ...
    console.log(
      `  - Prisma Client 版本: ${require("@prisma/client/package.json").version}`,
    );
    ...
    console.log("  ⚠️  重要提示：");
    console.log(
      "  要验证 Json 类型支持，需要先在 schema.prisma 中将字段类型从 String 改为 Json",
    );
    ...
    console.log(
      "  - 是否相等:",
      JSON.stringify(testData) === JSON.stringify(deserializedSettings),
    );
