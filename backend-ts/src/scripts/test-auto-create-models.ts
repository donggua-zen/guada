...
async function testAutoCreateModels() {
  ...
  try {
    ...
    models.forEach((model, index) => {
      ...
      console.log(`      特性: ${(model.features as any[])?.join(', ') || '无'}\n`);
    });
    ...
    try {
      console.log('\n🧹 尝试清理测试数据...');
      await prisma.$executeRawUnsafe('DELETE FROM model WHERE providerId IN (SELECT id FROM model_provider WHERE userId LIKE "test_%")');
      await prisma.$executeRawUnsafe('DELETE FROM model_provider WHERE userId LIKE "test_%"');
      await prisma.$executeRawUnsafe('DELETE FROM user WHERE email LIKE "test_%"');
      console.log('✅ 清理完成');
    } catch (cleanupError) {
      console.error('⚠️  清理失败:', cleanupError instanceof Error ? cleanupError.message : String(cleanupError));
    }
